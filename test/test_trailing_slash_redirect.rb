...
class TestTrailingSlashRedirect < Test::Unit::TestCase
  ...
  def test_returns_301_when_trailing_slash_on_path_info
    app = Proc.new { [200, {'Content-Type' => 'text/html'}, ['Downstream app']]}
    middleware = Rackables::TrailingSlashRedirect.new(app)
    status, headers, body = middleware.call({'rack.url_scheme' => 'http', 'HTTP_HOST' => 'bar.com', 'PATH_INFO' => '/foo/', 'SERVER_PORT' => 80})
  ...
  def test_301_respects_query_string
    app = Proc.new { [200, {'Content-Type' => 'text/html'}, ['Downstream app']]}
    middleware = Rackables::TrailingSlashRedirect.new(app)
    status, headers, body = middleware.call({'rack.url_scheme' => 'http', 'HTTP_HOST' => 'bar.com', 'PATH_INFO' => '/foo/', 'QUERY_STRING' => 'baz=hi', 'SERVER_PORT' => 80})
    assert_equal 301, status
  ...
  def test_301_respects_https
    app = Proc.new { [200, {'Content-Type' => 'text/html'}, ['Downstream app']]}
    middleware = Rackables::TrailingSlashRedirect.new(app)
    status, headers, body = middleware.call({'rack.url_scheme' => 'https', 'HTTP_HOST' => 'bar.com', 'PATH_INFO' => '/foo/', 'QUERY_STRING' => 'baz=hi', 'SERVER_PORT' => 443})
    assert_equal 301, status
